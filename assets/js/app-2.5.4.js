...
(function (iCafeTIKSpeedTest) {
  ...
  var iCafeTIKSpeedTestShow = function () {
    ...
    this.dlUnit = _("dlUnit");
    this.ulUnit = _("ulUnit");
  ...
  iCafeTIKSpeedTestShow.prototype.uploadResult = function (upload) {
    ...
    if (this.ulUnit) this.ulUnit.el.textContent = unit;
  ...
  iCafeTIKSpeedTestShow.prototype.downloadResult = function (download) {
    ...
    if (this.dlUnit) this.dlUnit.el.textContent = unit;
  ...
  iCafeTIKSpeedTestShow.prototype.LiveSpeed = function (data, Display) {
    ...
    if (Display === "Download" || Display === "Upload") {
      if (ShowData >= 1000) {
    ...
    if (Display === "Ping") {
      ...
      // Gauge Updates (Use original data for gauge scaling?)
      // The original code passed 'ShowData' which was raw data.
      // Now ShowData is scaled. 
      // If we pass scaled data to gauge, gauge will be tiny.
      // We should probably verify if Gauge needs original data.
      // Assuming Gauge expects Mbps.
      ...
      // Unit Updates
      if (Display === "Download") {
        this.downResult.el.textContent = ShowData.toFixed(1);
        if (this.dlUnit) {
          this.dlUnit.el.textContent = unit;
          // Optional: Adjust font size for Gbps to fit?
        }
      } else if (Display === "Upload") {
        this.upRestxt.el.textContent = ShowData.toFixed(1);
        if (this.ulUnit) {
          this.ulUnit.el.textContent = unit;
        }
      }

      // Gauge top speed update
      // Note: We used original 'data' for these checks before.
      // Let's use 'data' (raw) for gauge scaling checks vs 'ShowData' (display).
      if (data <= 1000) {
        this.oDoTopSpeed.el.textContent = "1000+";
        this.oDoTopSpeed.el.style.fontSize = "16.9px";
        this.oDoTopSpeed.el.style.fill = "gray";
      ...
      if (data >= 1010) {
        this.oDoTopSpeed.el.textContent = Math.floor(data / 1010) * 1000 + "+";
        this.oDoTopSpeed.el.style.fill = "gray";
    ...
    function iCafeTIKSpeedTest() {
      ...
      var Engine = setInterval(function () {
        ...
        if (Status === "Error") {
          ...
          dummyElement.innerHTML = '<a xlink:href="#" style="cursor: default" target="_self"></a>';
        ...
        if (xhr.readyState == 4 && xhr.status == 200) {
          ...
          if (auth == 3) {
            setTimeout(function () {
              ...
              console.log("Redirect blocked: " + showResult + return_data);
            }, 1500);
          }
          if (auth == 6) {
